//----
//
// This Code is Used for JwTable Component
...
function checkthisl(prevname,nextname){
    if(event.keyCode==27){window.close();return;}

    if(event.keyCode==37 ){ eval("document.forms(0)."+prevname+".focus()");return;};
    if(event.keyCode==39 ){ eval("document.forms(0)."+nextname+".focus()");return;};

}
			onkeyup="if(event.keyCode==37){document.forms(0).datedown.focus();return;};if(event.keyCode==27){window.close();return;};if(event.keyCode==39 || event.keyCode==13){document.forms(0).minutedown.focus();return;};if (event.keyCode <46 || event.keyCode >57) {this.value='';}"		

function checkthisln(prevname,nextname){
    if(event.keyCode==27){window.close();return;}

    if(event.keyCode==37 ){ eval("document.forms(0)."+prevname+".focus()");return;};
    if(event.keyCode==39 ){ eval("document.forms(0)."+nextname+".focus()");return;};
    if(event.keyCode <46 || event.keyCode >57) { this.value=''; return;}   
}
...
function _control_onkeydown() {
    ...
    if(element.getAttribute("ronly")=="t"){
		event.returnValue=false;
    }
    if(event.keyCode==27){
		if(confirm("请确认要退出吗？")){
    ...
    if(element.getAttribute("attrib")=="editor"){
       tabstr = element.getAttribute("tabindex");
       var tabnum = parseInt(tabstr);

       if(element.getAttribute("cnumber")=="t"){
          if (event.keyCode !=8){
              if (event.keyCode <46 || event.keyCode >57) event.returnValue = false;
          }
       }
       if(element.getAttribute("pnumber")=="t"){
          if ((event.keyCode !=8) && (event.keyCode !=109) && (event.keyCode !=190)){
              if (event.keyCode <46 || event.keyCode >57) event.returnValue = false;
          }
       }
       if(element.getAttribute("dropdown_mode")=="staticlist"){
          if(event.keyCode==39){
			 processTab(element);
          }
          if(event.keyCode==37){
             if(tabnum>1) tabnum --;  
             eval("document.forms(0).tab"+tabnum+".focus()");
          }
       }else{
          if(event.keyCode==40){
             if(tabnum < _pmaxtabnum){ 
                tabnum ++;  
                eval("document.forms(0).tab"+tabnum+".focus()");
             }
          }
          if(event.keyCode==38){
            if (!compareText(element.tagName, "textarea")){
               if(tabnum>1) tabnum --;  
               eval("document.forms(0).tab"+tabnum+".focus()");
            }
          }
	...
	if (isDropdownBoxVisible()){
		if (_dropdown_window){ 
		   _dropdown_window.processDropDownKeyDown(event.keyCode);
           if(event.keyCode==13){
			 processTab(element);
           }
        }
...
function adjustControlsSize(){
	function showAD(helptit){
		_ad_count++;
		if (!_ad_box || _ad_count>_ad_interval){
			if (_ad_box) _ad_box.removeNode(true);
			_ad_box=document.createElement("<A target='_blank' href='/btcjerpWeb/jsp/help/si/"+helptit+"'.html' style='color: blue; font-size: 9pt; position: absolute'></A>");
			_ad_box.innerText="博通软件";
    ...
    if(window.opener.parent.helptit){
	   if(helptitsub){
	     showAD(window.opener.parent.helptit + "_"+ helptitsub);
	   }else{
	     showAD(window.opener.parent.helptit);
       }
    }
}
...
function checkbox_autoDropDown_onclick() {
	window.document.forms(0).dropdown_1.setAttribute("autoDropDown", "ON");
	window.document.forms(0).dropdown_2.setAttribute("autoDropDown", "ON");
	window.document.forms(0).dropdown_3.setAttribute("autoDropDown", "ON");
}

function checkbox_editable_onclick() {
	window.document.forms(0).dropdown_1.setAttribute("dropDown_fixed", "ON");
	window.document.forms(0).dropdown_2.setAttribute("dropDown_fixed", "ON");
	window.document.forms(0).dropdown_3.setAttribute("dropDown_fixed", "ON");
	checkbox_restrict.disabled=checkbox_editable.checked;
}

function checkbox_restrict_onclick() {
	window.document.forms(0).dropdown_1.setAttribute("dropDown_restrict", "ON");
	window.document.forms(0).dropdown_2.setAttribute("dropDown_restrict", "ON");
	window.document.forms(0).dropdown_3.setAttribute("dropDown_restrict", "ON");
}
